...
# Importacoes especificas por modulo
from scapy.layers.l2   import Ether, ARP
from scapy.layers.inet import IP, ICMP, TCP, UDP
from scapy.packet      import Raw

# DHCP - modulo separado
try:
...
class PacketAnalyzer:
    """Converte um pacote Scapy num dicionario normalizado."""
    ...
    def _parse_arp(self, packet, r):
        ...
        if op == 'request':
            r['summary'] = "ARP Request: MAC {} pergunta quem tem o IP {}".format(arp.hwsrc, arp.pdst)
        else:
            r['summary'] = "ARP Reply: IP {} esta no MAC {} (Destino: {})".format(arp.psrc, arp.hwsrc, arp.hwdst)
    ...
    def _parse_icmp(self, packet, r):
        icmp     = packet[ICMP]
        type_str = self.ICMP_TYPES.get(icmp.type, 'Type {}'.format(icmp.type))
        r['protocol'] = 'ICMP'
        r['details'].update({
            'type':     icmp.type,
            'type_str': type_str,
        ...
        if icmp.type in (0, 8):
            ...
            r['summary'] = "ICMP {} {} -> {}  id={} seq={}".format(
                type_str, src, dst, icmp.id, icmp.seq)
        elif icmp.type == 3:
            code_str = self.ICMP_UNREACH_CODES.get(icmp.code, 'code={}'.format(icmp.code))
            r['summary'] = "ICMP {} ({}) {} -> {}".format(type_str, code_str, src, dst)
        elif icmp.type == 11:
            r['summary'] = "ICMP Time Exceeded {} -> {}".format(src, dst)
        else:
            r['summary'] = "ICMP {} {} -> {}".format(type_str, src, dst)
    ...
    def _parse_tcp(self, packet, r):
        ...
        src, dst = r['src_ip'], r['dst_ip']
        base = "TCP {}:{} -> {}:{} [{}]".format(src, tcp.sport, dst, tcp.dport, flags_str)

        note = ''
        if 'S' in flags and 'A' not in flags:
            note = ' (SYN - inicio de ligacao)'
        elif 'S' in flags and 'A' in flags:
            note = ' (SYN-ACK - ligacao aceite)'
        elif 'F' in flags and 'A' in flags:
            note = ' (FIN-ACK - termino)'
        elif 'F' in flags:
            note = ' (FIN - termino)'
        elif 'R' in flags:
            note = ' (RST - reset)'
    ...
    def _parse_dhcp(self, packet, r):
        ...
        type_str = self.DHCP_MSG_TYPES.get(msg_type, 'Type {}'.format(msg_type))
        ...
        if packet.haslayer(BOOTP):
            ...
            if offered and offered != '0.0.0.0':
                ...
                r['summary'] = "DHCP {} -> IP oferecido: {}".format(type_str, offered)
            else:
                r['summary'] = "DHCP {}".format(type_str)
            for opt in dhcp.options:
    ...
    def _parse_http(self, packet, r, tcp, payload):
        ...
        try:
            ...
            r['summary']               = "HTTP {}".format(first_line)
            r['details']['first_line'] = first_line
        except Exception:
            r['summary'] = "HTTP {}:{} -> {}:{}".format(
